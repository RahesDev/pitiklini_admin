...
function Dashboard() {
  ...
  const handleVipBadgeChange = async (userId, currentBadge) => {
    try {
      const data = {
        apiUrl: apiService.changeVipBadgeStatus,
        payload: { _id: userId, vipBadge: currentBadge },
      };

      const response = await postMethod(data);

      if (response.status) {
        toast.success("VIP Badge Updated Successfully");
        getUserDetails(currentPage); // Reload list
      } else {
        toast.error("Failed to Update VIP Badge");
      }
    } catch (error) {
      toast.error("Error updating VIP Badge");
    }
  };
                              ...
                              <span className="plus_14_ff">{item.internalID}</span>
                                ...
                                {/* <td>
                                  <label className="switch">
                                    <input
                                      type="checkbox"
                                      checked={item.vipBadge === true}
                                      onChange={() =>
                                        handleVipBadgeChange(
                                          item._id,
                                          item.vipBadge
                                        )
                                      }

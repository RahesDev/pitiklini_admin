...
function Swaping() {
  ...
  useEffect(() => {
    getSwapDetails(currentPage, filterKeyword);
  }, [currentPage, filterKeyword]);

  const getSwapDetails = async (page = 1, keyword = "") => {
    var datas = {
      apiUrl: apiService.get_all_user_swap,
            ...
            <div className="px-4 transaction_padding_top">
              <div className="px-2 my-4 transaction_padding_top tops">
                <div className="headerss">
                  <span className="dash-head">Swaping History </span>
                  <div>
                    <input
                      className="filters"
                      ...
                      {Usersdataref.current.length > 0 ? (
                        Usersdataref.current.map((item, i) => (
                          <tr key={item._id}>
                            ...
                            <td>
                              <span className="plus_14_ff">{item.swapTo}</span>
                            </td>
                            <td>
                              <span className="plus_14_ff">{item.price}</span>
                            </td>
                            <td>
                            ...
                            <td>
                              <label className="plus_14_ff">
                                {parseFloat(item.fee).toFixed(4)}</label>
                            </td>
                            <td>
                              <span className="plus_14_ff">
